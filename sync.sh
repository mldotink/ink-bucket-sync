...
run_bisync_once() {
  ...
  if "${RCLONE_CMD[@]}" bisync "${SYNC_PATH}" "${REMOTE}" "${EXCLUDE_FLAGS[@]}" "${bisync_extra_flags[@]}" --create-empty-src-dirs --force --ignore-checksum --quiet 2>&1; then
    LAST_SYNC_EPOCH="${now}"
  elif [ "${use_resync}" != "true" ]; then
    log "Bisync failed, recovering with --resync"
    if "${RCLONE_CMD[@]}" bisync "${SYNC_PATH}" "${REMOTE}" "${EXCLUDE_FLAGS[@]}" --resync --create-empty-src-dirs --ignore-checksum --quiet 2>&1; then
      LAST_SYNC_EPOCH="${now}"
    else
      log "Resync failed (will retry)"
    fi
  else
